(async()=>{
  ...
  // Zonas de impresión más visibles y cómodas en móvil.
  HAKI.addPrintZone=()=>{
    ...
    const z=HAKI.addMeta(new fabric.Rect({
      ...
      fill:'rgba(0,240,255,.20)',
      stroke:'#00ffff',strokeWidth:8,strokeDashArray:null,strokeUniform:true,
      objectCaching:false,selectable:true,evented:true,
      transparentCorners:false,cornerColor:'#fff',cornerStrokeColor:'#00ffff',
      cornerStyle:'circle',cornerSize:22,borderColor:'#00ffff',borderScaleFactor:3
  ...
  // Warp manual corregido para conservar color y transparencia.
  HAKI.applyWarp=()=>{
    const o=HAKI.active();
    ...
    const minX=Math.min(...q.map(p=>p[0]));
    const minY=Math.min(...q.map(p=>p[1]));
    const maxX=Math.max(...q.map(p=>p[0]));
    ...
    fabric.Image.fromURL(off.toDataURL('image/png'),img=>{
  ...
  // Los controles de historial usan ahora la barra superior nativa.

  // Zoom con dos dedos directamente sobre el lienzo.
  if(!HAKI._pinchInstalled&&HAKI.canvas?.upperCanvasEl){
    ...
    el.addEventListener('touchstart',e=>{
      ...
      e.preventDefault();
      pinch={dist:Math.max(1,distance(e.touches)),zoom:HAKI.canvas.getZoom(),vpt:HAKI.canvas.viewportTransform.slice(),center:centerOf(e.touches)};
    },{passive:false});
    el.addEventListener('touchmove',e=>{
      if(!pinch||e.touches.length!==2)return;
      e.preventDefault();
      const ctr=centerOf(e.touches);
      const z=Math.max(.08,Math.min(8,pinch.zoom*(Math.max(1,distance(e.touches))/pinch.dist)));
      ...
      v[4]+=ctr.x-pinch.center.x;v[5]+=ctr.y-pinch.center.y;
  ...
  if(panel&&!document.getElementById('mobileBack')){
    ...
    back.id='mobileBack';back.type='button';back.textContent='← Volver al editor';
  ...
  if(b)b.innerHTML='<strong>HAKI STUDIO</strong><span>No se pudo cargar el editor. Revisa la conexión para cargar Fabric.js.</span>';
});
